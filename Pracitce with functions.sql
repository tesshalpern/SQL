...
-- Display the largest sales price 
SELECT MAX (price) FROM sales;

-- Display the average price of all sales
SELECT AVG (price) FROM sales;

-- Display the products sold in each city with total sales greater than $2,400
SELECT city, product, SUM(price)
FROM sales
GROUP BY city, product
HAVING SUM(price) >2400;

-- Show the average sale price in each city with an average sale price greater than $1,200
SELECT city, AVG(price) As Average_Sale
FROM sales
GROUP BY city
HAVING avg (price)  > 1200; 

-- Show transactions where payment type is one of two types and in a specific country
SELECT ID, name, country, payment_type
FROM sales 
WHERE (payment_type = "Mastercard" OR payment_type = "Amex") 
...
CASE
  WHEN price > 1200 THEN "large order"
  ELSE "small order"
END AS order_type
FROM sales;
